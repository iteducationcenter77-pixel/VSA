...
const defaultInstituteSettings: InstituteSettings = {
  ...
  admin_username: 'admin@vectora.edu',
...
export const VectoraProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const clearAllData = () => {
    setStudents([]);
    setAttendance([]);
    localStorage.removeItem('vsa_students');
    localStorage.removeItem('vsa_attendance');
    syncToVercelCloud([], []);
    addToast('System Cleared', 'All demo students & attendance records have been wiped clean.', 'success');
  };
